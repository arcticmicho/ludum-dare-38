...
public class EditorWWW
{
    ...
    public static EditorWWW RequestAsync(string url, System.Action<bool, string> callback)
    {
        return new EditorWWW(url, false, callback);
    }

    public static EditorWWW RequestAsync(string url, byte[] postData, System.Action<bool, string> callback)
    {
        return new EditorWWW(url, postData, false, callback);
    }

    public static EditorWWW RequestAsync(string url, byte[] postData, Dictionary<string, string> headers, System.Action<bool, string> callback)
    {
    ...
    public static EditorWWW RequestSync(string url, byte[] postData, System.Action<bool, string> callback)
    {
        return new EditorWWW(url, postData, true, callback);
    }

    public static EditorWWW RequestSync(string url, byte[] postData, Dictionary<string, string> headers, System.Action<bool, string> callback)
    {
        return new EditorWWW(url, postData, headers, true, callback);
    }

    private EditorWWW(string url, bool sync, System.Action<bool,string> callback)
    {
        _finished = false;
        _success = false;
        _callback = callback;
    ...
    private void FinishRequest(bool hasTimeout)
    {
        if (_request.isDone)
        {
            ...
            _callback.SafeInvoke(_success, response);
        }
    }
}
